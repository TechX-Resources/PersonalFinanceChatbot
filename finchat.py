...
# parse_simple_calc() and run_calc() same as old code
# ---------------------- Plot Helpers ----------------------
...
def parse_simple_calc(query: str, df):
    """
    Parse a simple calculation query like:
    - 'total spend at amazon'
    - 'average spend on groceries'
    - 'max transaction at walmart'
    Returns a recipe dict with {op, column, filter}
    """
    query = query.lower()
    ...
    for merchant in df["Description"].unique():
...
def run_calc(df, recipe):
    """
    Run a simple calculation (sum, avg, count, etc.) on the dataframe
    based on the recipe parsed from the query.
    """
    if not recipe or "operation" not in recipe:
        return {"error": "Invalid recipe"}

    op = recipe["operation"]
    column = recipe.get("column", "Amount")  # default to Amount column
    ...
    return {"operation": op, "column": column, "result": float(value)}
...
def handle_query(
    ...
    """
    Process a user query: detect intent, retrieve relevant rows, compute calc/plot/table, optionally ask LLM.
    """
